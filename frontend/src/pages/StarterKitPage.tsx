...
const IconSvg: React.FC<{ type: string; size: number }> = ({ type, size }) => {
  ...
  switch (type) {
    ...
    case 'pencil':
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className={className}>
          <path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l5.5-5.5L17 3z" />
        </svg>
      );
    case 'book':
      return (
...
const StarterKitPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 w-full overflow-y-scroll no-scrollbar bg-slate-50 flex flex-col relative">
