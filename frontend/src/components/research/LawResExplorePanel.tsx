...
interface LawResExplorePanelProps {
...
const LawResExplorePanel: React.FC<LawResExplorePanelProps> = ({
  lecturer,
  isExpanded,
  isInterested,
  isSaving,
  ...
  return (
    <div
      className={`border border-slate-200 rounded-xl overflow-hidden transition-all duration-300 cursor-pointer ${
        isExpanded ? 'shadow-lg ring-2 ring-amber-500/20' : 'hover:shadow-md hover:border-amber-300'
      }`}
      onClick={onToggle}
    >
          ...
          <div className="pt-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <a href={`mailto:${lecturer.email}`} className="text-amber-600 hover:underline" onClick={(e) => e.stopPropagation()}>
                  {lecturer.email}
                </a>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {lecturer.orcid && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 bg-amber-100 hover:bg-amber-200 text-amber-700 rounded-full transition-colors"
                  onClick={(e) => e.stopPropagation()}
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="pt-3 border-t border-slate-200">
              ...
              <div className="flex flex-wrap gap-1.5">
                {lecturer.researchAreas.map((area) => (
                  <span key={area} className="text-xs px-2 py-0.5 bg-amber-50 text-amber-700 rounded">
                    {area}
                  </span>
                ))}
              </div>
...
export default LawResExplorePanel;
