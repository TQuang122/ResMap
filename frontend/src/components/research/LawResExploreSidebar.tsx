...
interface LawResExploreSidebarProps {
  allAreas: string[];
...
const LawResExploreSidebar: React.FC<LawResExploreSidebarProps> = ({
  allAreas,
  selectedAreas,
  onToggleArea,
  onSelectAll,
    ...
    <div className="w-full sm:w-64 flex-shrink-0 bg-white border-r border-slate-200 p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1">
          <button
            onClick={onSelectAll}
            className="text-xs text-amber-600 hover:text-amber-700 font-medium px-1.5 py-0.5 hover:bg-amber-50 rounded"
          >
            Chọn tất cả
          </button>
            ...
            >
              <div className="relative flex items-center">
                <input
                  ...
                  className="peer w-4 h-4 border-2 border-slate-300 rounded text-amber-600 focus:ring-amber-500 focus:ring-offset-0 cursor-pointer appearance-none checked:border-amber-600 checked:bg-amber-600 transition-colors"
                />
                <svg
...
export default LawResExploreSidebar;
