...
interface LanguagesResExploreSidebarProps {
  allAreas: string[];
...
const LanguagesResExploreSidebar: React.FC<LanguagesResExploreSidebarProps> = ({
  allAreas,
  selectedAreas,
  onToggleArea,
  onSelectAll,
    ...
    <div className="w-full sm:w-64 flex-shrink-0 bg-white border-r border-slate-200 p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-1">
          <button
            onClick={onSelectAll}
            className="text-xs text-violet-600 hover:text-violet-700 font-medium px-1.5 py-0.5 hover:bg-violet-50 rounded"
          >
            Chọn tất cả
          </button>
            ...
            >
              <div className="relative flex items-center">
                <input
                  ...
                  className="peer w-4 h-4 border-2 border-slate-300 rounded text-violet-600 focus:ring-violet-500 focus:ring-offset-0 cursor-pointer appearance-none checked:border-violet-600 checked:bg-violet-600 transition-colors"
                />
                <svg
...
export default LanguagesResExploreSidebar;
