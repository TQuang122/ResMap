...
import BusinessResExploreSidebar from './BusinessResExploreSidebar';
import BusinessResExplorePanel from './BusinessResExplorePanel';

interface BusinessResExploreModalProps {
  isOpen: boolean;
  onClose: () => void;
  lecturers: LecturerData[];
}

const BusinessResExploreModal: React.FC<BusinessResExploreModalProps> = ({ isOpen, onClose, lecturers }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-6xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden animate-slideUp flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Business ResExplore</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              Khám phá giảng viên Kinh doanh, Quản trị & Tài chính tại FPT University
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:block">
              <input
                ...
                className="w-64 pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              />
              <svg
...
export default BusinessResExploreModal;
